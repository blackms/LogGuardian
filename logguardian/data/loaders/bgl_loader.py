"""
Loader for BGL (Blue Gene/L) supercomputer logs dataset.
"""
import os
import re
import pandas as pd
...
class BGLLoader(BaseDataLoader):
    """
    Loader for the BGL (Blue Gene/L) supercomputer logs dataset.
    
    The BGL dataset contains logs collected from a BlueGene/L supercomputer system 
    at Lawrence Livermore National Labs (LLNL). Each log message is labeled as 
    either normal or anomalous.
    
    BGL log format example:
    ...
    def _parse_bgl_logs(self, filepath: str) -> Tuple[List[str], List[int], List[float]]:
        """
        Parse BGL logs from file.
        
        Args:
            filepath: Path to BGL log file
            
        Returns:
            Tuple of (log_messages, labels, timestamps)
        """
        # Regular expression to extract BGL log parts
        bgl_regex = r'(\d+)\s+([\d\.]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+(\w+)\s+(\w+)\s+(\w+)\s+(.*)'
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    match = re.match(bgl_regex, line.strip())
                    if match:
                        ...
                        # In BGL, "FATAL", "ERROR", "SEVERE", "WARNING" indicate anomalies
    ...
    def load(self, path: Optional[str] = None) -> Tuple[List[List[str]], List[int]]:
        """
        Load and process BGL logs.
        
        Args:
            path: Path to BGL log file (overrides config path)
        ...
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"BGL log file not found at {filepath}")
        
        logger.info(f"Loading BGL logs from {filepath}")
        
        # Parse logs
        logs, labels, timestamps = self._parse_bgl_logs(filepath)
    ...
    def get_train_test_split(
        ...
        For BGL, the split is chronological by default, where the first
        ...
        if shuffle:
            logger.warning("Shuffling data for BGL might not be realistic for time-series data.")
