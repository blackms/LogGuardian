"""
Loader for Thunderbird supercomputer logs dataset.
"""
import os
import re
import pandas as pd
...
class ThunderbirdLoader(BaseDataLoader):
    """
    Loader for the Thunderbird supercomputer logs dataset.
    
    The Thunderbird dataset contains logs collected from the Thunderbird supercomputer
    at Sandia National Laboratories (SNL). Each log message is labeled
    as either normal or anomalous.
    
    Thunderbird log format example:
    1131574520 2005.11.09 R26-M0-N2-C:J12-U11 2005-11-09-21.02.00.766233 R26-M0-N2-C:J12-U11 RAS APP FATAL ciod: Error reading message prefix on CioStream socket.ConvertInt: Unrecognized tag #
    """
    ...
    def _parse_thunderbird_logs(self, filepath: str) -> Tuple[List[str], List[int], List[float]]:
        """
        Parse Thunderbird logs from file.
        
        Args:
            filepath: Path to Thunderbird log file
            
        Returns:
            Tuple of (log_messages, labels, timestamps)
        """
        # Regular expression to extract Thunderbird log parts
        # Thunderbird logs have the same format as BGL logs
        thunderbird_regex = r'(\d+)\s+([\d\.]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+(\w+)\s+(\w+)\s+(\w+)\s+(.*)'
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    match = re.match(thunderbird_regex, line.strip())
                    if match:
                        ...
                        # In Thunderbird, "FATAL", "ERROR", "SEVERE", "WARNING" indicate anomalies
    ...
    def load(self, path: Optional[str] = None) -> Tuple[List[List[str]], List[int]]:
        """
        Load and process Thunderbird logs.
        
        Args:
            path: Path to Thunderbird log file (overrides config path)
        ...
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"Thunderbird log file not found at {filepath}")
        
        logger.info(f"Loading Thunderbird logs from {filepath}")
        
        # Parse logs
        logs, labels, timestamps = self._parse_thunderbird_logs(filepath)
    ...
    def get_train_test_split(
        ...
        For Thunderbird, the split is chronological by default, where the first
        ...
        if shuffle:
            logger.warning("Shuffling data for Thunderbird might not be realistic for time-series data.")
